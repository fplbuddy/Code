...
! of the fields and external force matrixes
      ...
      COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: meantheta, meanps
!
...
! Allocates memory for distributed blocks
      ...
      ALLOCATE( meanps(ny,ista:iend) )
      ALLOCATE( meantheta(ny,ista:iend) )
...
! Runge-Kutta step 3
         ...
         DO i = ista,iend
            DO j = 1,ny
              meanps(j,i) = meanps(j,i) + ps(j,i)*dt
              meantheta(j,i) = meantheta(j,i) + theta(j,i)*dt
            END DO
         END DO
...
! devide by total time
DO i = ista,iend
   DO j = 1,ny
     meanps(j,i) = meanps(j,i)/time
     meantheta(j,i) = meantheta(j,i)/time
   END DO
END DO

! print the mean fieldsfs
CALL fieldsfs(meanps,meantheta,'mean')
