...
SUBROUTINE spectrum(a,ext)
  ...
  COMPLEX(KIND=GP), INTENT(IN), DIMENSION(ny,ista:iend) :: a
  REAL, DIMENSION(specmax) :: Ek,Ektot
...
SUBROUTINE spectrum2D(a,ext)
  ...
  REAL, DIMENSION(nmax/2+1,nmax/2+1) :: Ek,Ektot !nmax NOT sure
  COMPLEX(KIND=GP), INTENT(IN), DIMENSION(ny,ista:iend) :: a
  REAL        :: tmp,two
  INTEGER     :: kin
  INTEGER     :: kmnx,kmny
...
SUBROUTINE vectrans(a,b,c,ext1,ext2)
  ...
  REAL, DIMENSION(nmax/2+1) :: Ek,Ektot
  REAL(KIND=GP)    :: tmp
  INTEGER          :: kmn,kin
  INTEGER          :: i,j
...
!*****************************************************************
SUBROUTINE CFL_condition(a,Ly, Lx,cfl,ord,dt)
  !-----------------------------------------------------------------
  !
  ...
  !dx = 3.0_GP/(pi*real(nx,kind=GP)*Ly) ! not sure about this, 3 makes sense, see line above, but not sure where the pi comes from? ... If we trust V that dx_dim = nx/3, then this is correct! Just mupltipy by 1/(pi Ly). But i think that dx_dim = 2*Lx*pi*nx/3, should be correct surely...
  dx = min(3.0_GP*2.0_GP*Lx/(real(nx,kind=GP)*Ly), 3.0_GP/(real(ny,kind=GP)*2.0_GP))
  ...
  ! Have some notes on the above. Basivally Umax_nd = Umax_d/(nu/piLy), dx_nd = dx_d/(pi Ly). What we have above are both dx_nd and Umax_nd, but we also need to mutlipy by the timescale. ie t_nd = t_d*nu/(pi Ly)^2.
  RETURN
