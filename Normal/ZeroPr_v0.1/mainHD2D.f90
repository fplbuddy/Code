...
PROGRAM HD2D
  ...
  RK : DO t = ini,step
      ...
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN
            C1(j,i) = (C1(j,i) + dt*rmp*( C3(j,i)/kk2(j,i)-Ra*im*kx(i)*C2(j,i)/kk2(j,i))) &
            /(1.0d0 + kk2(j,i)*dt*rmp)

            C2(j,i) = im*kx(i)*C1(j,i)/kk2(j,i)
          ELSE
            C1(j,i) = 0.0d0
            C2(j,i) = 0.0d0
          ENDIF
    ...
    DO i = ista,iend
      DO j = 1,ny
        IF (kn2(j,i).le.kmax) THEN
          ps(j,i) = (ps(j,i) + dt*rmp*( C3(j,i)/kk2(j,i)-Ra*im*kx(i)*C2(j,i)/kk2(j,i))) &
          /(1.0d0 + kk2(j,i)*dt*rmp)

          theta(j,i) = im*kx(i)*ps(j,i)/kk2(j,i)
        ELSE
          ps(j,i) = 0.0d0
          theta(j,i) = 0.0d0
        ENDIF
