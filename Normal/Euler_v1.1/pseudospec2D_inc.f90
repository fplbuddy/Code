...
SUBROUTINE hdcheck(a,b,t,ordvf,ordvh,znl)
  ...
  DOUBLE PRECISION :: enk,denk,vth, Ey, Ex, enke, enko
  ...
  IF (myrank.eq.0) THEN
    ...
    WRITE(1,20) t,enk,denk,vth,Ey,Ex, enke, enko
...
!*****************************************************************
SUBROUTINE CFL_condition(a,cfl,ord,vord,dt)
  !-----------------------------------------------------------------
  !
  ...
  if (ord.eq.2) then
    dt = cfl*(sqrt(3.0d0)/pi)/kcut/umax! Phil: have not added temp dep here
  else if (ord.eq.3) then
    dt = cfl*(sqrt(3.0d0)/pi)/(kcut*umax)
  end if
