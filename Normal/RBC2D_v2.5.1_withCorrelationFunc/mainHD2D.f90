...
! of the fields and external force matrixes
      ...
      COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: ps, psstart
      COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: theta, thetastart
!
...
! Allocates memory for distributed blocks
      ...
      ALLOCATE( thetastart(ny,ista:iend) )
      ALLOCATE( psstart(ny,ista:iend) )
...
! with the power spectrum
         IF (times.eq.sstep) THEN
            ...
            CALL CorFunc(psstart,ps,thetastart,theta,ext4)
            IF (znl.eq.1) THEN
