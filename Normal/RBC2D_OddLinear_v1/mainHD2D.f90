...
RK : DO t = ini,step
  ...
  DO o = ord,2,-1
    ...
    DO i = ista,iend
      DO j = 1,ny
        IF (kn2(j,i).le.kmax.and.MOD(i+j,2).eq.0) THEN ! only evolve odd modes
          C1(j,i) = (ps(j,i) + dt*rmp*( C3(j,i)/kk2(j,i)-im*kx(i)*C2(j,i)/kk2(j,i))) &
          /(1.0d0 + (nu*kk2(j,i))*dt*rmp)

          C2(j,i) = (theta(j,i) + dt*rmp*(-C4(j,i)+im*kx(i)*C1(j,i)/pi)) &
          /(1.0d0 + (kappa*kk2(j,i))*dt*rmp)
        ELSE IF (kn2(j,i).gt.kmax)
          C1(j,i) = 0.0d0
          C2(j,i) = 0.0d0
        ENDIF
  ...
  DO i = ista,iend
    DO j = 1,ny
      IF (kn2(j,i).le.kmax.and.MOD(i+j,2).eq.0) THEN
        ps(j,i) = (ps(j,i)+ dt*rmp*( C3(j,i)/kk2(j,i)-im*kx(i)*C2(j,i)/kk2(j,i))) &
        /(1.0d0 + (nu*kk2(j,i))*dt*rmp)
