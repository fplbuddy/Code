...
PROGRAM HD2D
  ...
  ! Calculating nu and kappa, and dt as we are not doing CFL check in this case
  nu = sqrt((pi*Ly)**3*Pr/Ra)
  kappa = sqrt((pi*Ly)**3/(Ra*Pr))
  kcut = real(nx,kind=GP)/3.0_GP
  dt = (2.5d0/pi**2)/(nu*kcut**2) ! will always be the smallest time scale in this case, saves us having do CFL each time
  ...
  IF (stat.eq.0) THEN
    ...
    ELSE IF (InC.eq.5) THEN
      ...
      ALLOCATE( Rpsi(2500), Ipsi(2500), Rtheta(2500), Itheta(2500) ) ! Allocation memory to get the ICs, length of the IC vector in r+ij form
      ...
      DO i = ista,iend ! Setting to 0 and then adding ICs
        DO j = 1,ny
          ps(j,i) = 0.0d0
          theta(j,i) = 0.0d0
          IF (MOD(i+j,2).eq.1.and.i.lt.51.and.j.lt.101) THEN ! Checking that mode is even and that we are within the bounds of PsiE and ThetaE
            ps(j,i) = real(Rpsi((j-1)*25+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP) ! The number here is just N/4
            theta(j,i) = real(Rtheta((j-1)*25+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
            IF (i.ne.1) THEN ! If we have an imaginary part also
              ps(j,i) = ps(j,i) + im*real(Ipsi((j-1)*25+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
              theta(j,i) = theta(j,i) + im*real(Itheta((j-1)*25+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
            END IF
          END IF
    ...
    ELSE IF (InC.eq.6) Then
      ...
      ElSE IF (convs.eq.2) THEN ! load in from MATLAB
        ! These number have been set for 172
        ALLOCATE( Rpsi(5776), Ipsi(5776), Rtheta(5776), Itheta(5776) ) ! Allocation memory to get the ICs, length of the IC vector in r+ij form
        ...
        DO i = ista,iend ! Setting to 0 and then adding ICs
          DO j = 1,ny
            ps(j,i) = 0.0d0
            theta(j,i) = 0.0d0
            IF (MOD(i+j,2).eq.1.and.i.lt.77.and.j.lt.153) THEN ! Checking that mode is even and that we are within the bounds of PsiE and ThetaE
              ps(j,i) = real(Rpsi((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP) ! The number here is just N/4
              theta(j,i) = real(Rtheta((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
              IF (i.ne.1) THEN ! If we have an imaginary part also
                ps(j,i) = ps(j,i) + im*real(Ipsi((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
                theta(j,i) = theta(j,i) + im*real(Itheta((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
              END IF
            END IF
  ...
  END DO
    ...
    ELSE IF (InC.eq.7) THEN ! steady state with odd and even modes from matlab
      ...
      ! mmax*(nmax+1)
      ALLOCATE( Rpsi(924), Ipsi(924), Rtheta(924), Itheta(924) ) ! Allocation memory to get the ICs, length of the IC vector in r+ij form
      ...
      DO i = ista,iend ! Setting to 0 and then adding ICs
        DO j = 1,ny
          ps(j,i) = 0.0d0
          theta(j,i) = 0.0d0
          IF (i.lt.23.and.j.lt.43) THEN ! Checking that mode is even and that we are within the bounds of PsiE and ThetaE
            ps(j,i) = real(Rpsi(i + 22*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP) ! The number here is just N/4
            theta(j,i) = real(Rtheta(i + 22*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
            IF (i.ne.1) THEN ! If we have an imaginary part also
              ps(j,i) = ps(j,i) + im*real(Ipsi(i + 22*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
              theta(j,i) = theta(j,i) + im*real(Itheta(i + 22*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
            END IF
          END IF
        END DO
      END DO
  ...
  ELSE IF (pert.eq.2) THEN ! Adding eigenfunction from matlab
    ...
    DO i = ista,iend ! adding the part
      DO j = 1,ny
        IF (MOD(i+j,2).eq.0.and.i.lt.77.and.j.lt.153) THEN ! Checking that mode is odd and that we are within the bounds of PsiV and ThetaV
          ps(j,i) = real(Rpsi((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts ! The number here is just N/4
          theta(j,i) = real(Rtheta((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts
          IF (i.ne.1) THEN ! If we have an imaginary part also
            ps(j,i) = ps(j,i) + im*real(Ipsi((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts
            theta(j,i) = theta(j,i) + im*real(Itheta((j-1)*38+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts
          END IF
        END IF
