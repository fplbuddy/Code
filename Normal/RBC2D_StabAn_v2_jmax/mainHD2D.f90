...
PROGRAM HD2D
  ...
  !
   ...
   ELSE IF (InC.eq.3) THEN ! Matlab
     ALLOCATE( Rpsi(16384), Ipsi(16384), Rtheta(16384), Itheta(16384) ) ! Allocation memory to get the ICs, length of the IC vector in r+ij form
     ...
     DO i = ista,iend ! Setting to 0 and then adding ICs
       DO j = 1,ny
         ps(j,i) = 0.0d0
         theta(j,i) = 0.0d0
         IF (MOD(i+j,2).eq.1.and.i.lt.129.and.j.lt.257) THEN ! Checking that mode is even and that we are within the bounds of PsiE and ThetaE
           ps(j,i) = real(Rpsi((j-1)*64+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP) ! The number here is just  N/4
           theta(j,i) = real(Rtheta((j-1)*64+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
           IF (i.ne.1) THEN ! If we have an imaginary part also
             ps(j,i) = ps(j,i) + im*real(Ipsi((j-1)*64+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
             theta(j,i) = theta(j,i) + im*real(Itheta((j-1)*64+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
           END IF
         END IF
   ...
   DO WHILE (abs(delta).gt.convlim.and.ESD.eq.0)        !
    ...
    DO i = ista,iend
      IF (mod(i,2).eq.0) THEN ! i is even, ie it is and odd number
          jmin = 2 ! So we wawnt j to even
      ELSE
        jmin = 1
      END IF
      DO j = jmin,ny,2
        IF (kn2(j,i).le.kmax) THEN ! Adding a small odd pertubation
          rannum = 2*pi*randu(seed)*kx(i)
    ...
    RK : DO t = ini,step
      ...
      !
      ! Checks the necessary CFL condition for numerical stability
      CALL CFL_condition(ps,nu,kappa,CFL,ord,ordvf,dt,0)
      ...
      DO i = ista,iend
        ...
        ELSE
          jmin = 1
        END IF
        DO j = jmin,jmax,2 ! jmax = int(ny*2/3)
          IF (kn2(j,i).le.kmax) THEN
            ps(j,i) = (ps(j,i)+ dt*rmp*( C3(j,i)/kk2(j,i)-im*kx(i)*C2(j,i)/kk2(j,i))) &
            /(1.0d0 + (nu*kk2(j,i))*dt*rmp)
