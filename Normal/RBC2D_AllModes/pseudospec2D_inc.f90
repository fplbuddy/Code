...
!
      ...
      DOUBLE PRECISION, DIMENSION((kxmax + 1),kymax) :: Ekreal, Ekrealtot, Ekimag, Ekimagtot, Epreal, Eprealtot, Epimag, Epimagtot !The + 1 is there since we have 0 in kx
      COMPLEX(KIND=GP), INTENT(IN), DIMENSION(ny,ista:iend) :: a, b
      ...
      DO i = ista,iend
         ...
         DO j = 1,ny
            ...
            IF ((kmny.gt.0).and.(kmny.le.(2*ny/3+1))) THEN
               Ekreal(kmnx,kmny) = REAL(a(j,i))*nrm
               Ekimag(kmnx,kmny) = AIMAG(a(j,i))*nrm
               Epreal(kmnx,kmny) = REAL(b(j,i))*nrm
               Epimag(kmnx,kmny) = AIMAG(b(j,i))*nrm
            ENDIF
            ENDIF
         END DO
      END DO
      CALL MPI_REDUCE(Ekreal,Ekrealtot,(kxmax + 1)*kymax,GC_REAL,MPI_SUM, &
                      0,MPI_COMM_WORLD,ierr)
      CALL MPI_REDUCE(Ekimag,Ekimagtot,(kxmax + 1)*kymax,GC_REAL,MPI_SUM, &
                                      0,MPI_COMM_WORLD,ierr)
      CALL MPI_REDUCE(Epreal,Eprealtot,(kxmax + 1)*kymax,GC_REAL,MPI_SUM, &
                                                      0,MPI_COMM_WORLD,ierr)
      CALL MPI_REDUCE(Epimag,Epimagtot,(kxmax + 1)*kymax,GC_REAL,MPI_SUM, &
                                                                      0,MPI_COMM_WORLD,ierr)
      IF (myrank.eq.0) THEN
         OPEN(1,file=trim(sdir) // '/spectrum2D_UUreal.' // ext // '.out',form='unformatted')
         WRITE(1) Ekrealtot
         CLOSE(1)
         OPEN(1,file=trim(sdir) // '/spectrum2D_UUimag.' // ext // '.out',form='unformatted')
         WRITE(1) Ekimagtot
         OPEN(1,file=trim(sdir) // '/spectrum2D_PPreal.' // ext // '.out',form='unformatted')
         WRITE(1) Eprealtot
         CLOSE(1)
         OPEN(1,file=trim(sdir) // '/spectrum2D_PPimag.' // ext // '.out',form='unformatted')
         WRITE(1) Epimagtot
      ENDIF
...
!!!      kcut = max(real(nx,kind=GP)/lx, 2.0_GP*real(ny,kind=GP)/ly)/3.0_GP
      ...
      !dx = min(Ly*pi*3.0_GP/(2.0_GP*real(ny,kind=GP)), Lx*pi*3.0_GP/(real(nx,kind=GP)))
