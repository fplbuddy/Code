...
PROGRAM HD2D
  ...
  !
  IF (stat.eq.0) THEN
ALLOCATE( Rpsi(size), Ipsi(size), Rtheta(size), Itheta(size) )
...
DO i = ista,iend ! Setting to 0 and then adding ICs
  DO j = 1,ny
    ps(j,i) = 0.0d0
    theta(j,i) = 0.0d0
    IF (i.lt.(nx/2+1).and.j.lt.(ny+1)) THEN ! Checking that mode is even and that we are within the bounds of PsiE and ThetaE, THINK THIS IS WRONG
      ps(j,i) = real(Rpsi(i + (nx/4)*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP) ! The number here is just N/4
      theta(j,i) = real(Rtheta(i + (nx/4)*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
      IF (i.ne.1) THEN ! If we have an imaginary part also
        ps(j,i) = ps(j,i) + im*real(Ipsi(i + (nx/4)*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
        theta(j,i) = theta(j,i) + im*real(Itheta(i + (nx/4)*(j-1)),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
      END IF
    END IF
  END DO
END DO
