...
PROGRAM HD2D
  ...
  REAL(KIND=GP) :: delta,convlim ! Used for convergance
  REAL(KIND=GP) :: time
  REAL(KIND=GP) :: rannum
  !REAL(KIND=GP) :: cort
  ...
  INTEGER :: mult,stat, InC, pert, norm, convs
  INTEGER :: ordvf,ordvh
  INTEGER :: t,o
  INTEGER :: i,j,ir,jr
  INTEGER :: ic,id,iu,ix
  ...
  IF (stat.eq.0) THEN
    ...
    ELSE IF (InC.eq.5) THEN
      ...
      ALLOCATE( Rpsi(7396), Ipsi(7396), Rtheta(7396), Itheta(7396) ) ! Allocation memory to get the ICs, length of the IC vector in r+ij form
      ...
      DO i = ista,iend ! Setting to 0 and then adding ICs
        DO j = 1,ny
          ps(j,i) = 0.0d0
          theta(j,i) = 0.0d0
          IF (MOD(i+j,2).eq.1.and.i.lt.87.and.j.lt.173) THEN ! Checking that mode is even and that we are within the bounds of PsiE and ThetaE
            ps(j,i) = real(Rpsi((j-1)*43+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP) ! The number here is just N/4
            theta(j,i) = real(Rtheta((j-1)*43+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
            IF (i.ne.1) THEN ! If we have an imaginary part also
              ps(j,i) = ps(j,i) + im*real(Ipsi((j-1)*43+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
              theta(j,i) = theta(j,i) + im*real(Itheta((j-1)*43+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
            END IF
          END IF
    ...
    ELSE IF (InC.eq.6) Then
      IF (convs.eq.1) THEN ! Read in ICs
    ...
    DO i = ista,iend
      DO j = 1,ny
        IF (i.eq.1.and.j.eq.1.and.pert.eq.1) THEN ! Adding a small odd pertubation
          ps(j,i) = real(perts*real(ny,kind=GP)*real(nx,kind=GP),kind = GP )
          theta(j,i) = real(perts*real(ny,kind=GP)*real(nx,kind=GP),kind = GP )
        END IF
