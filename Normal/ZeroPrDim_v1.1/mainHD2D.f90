...
PROGRAM HD2D
  ...
  COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: C1,C3, C4
  ...
  IF (stat.eq.0) THEN
      ...
      DO j = jsta,jend
         DO i = 1,ny
            R1(i,j) = R1(i,j) + sin(2*pi*ir*(dble(i)-1)/dble(nx)+phase1) &
                              * sin(pi*ir*(dble(j)-0.5d0)/dble(ny)+phase2)
         END DO
      END DO
    ...
    RK : DO t = ini,step
        ...
        DO i = ista,iend
          DO j = 1,ny
            IF (kn2(j,i).le.kmax) THEN
              C1(j,i) = (ps(j,i) + dt*rmp*( C3(j,i)/kk2(j,i)+(kx(i)**2)*C1(j,i)/(pi*(kk2(j,i)**2)))) &
      ...
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN
            ps(j,i) = (ps(j,i)+ dt*rmp*( C3(j,i)/kk2(j,i)+(kx(i)**2)*C1(j,i)/(pi*(kk2(j,i)**2)))) &
            /(1.0d0 + (nu*kk2(j,i))*dt*rmp)
