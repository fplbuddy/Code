...
PROGRAM HD2D
  ...
  CALL MPI_BCAST(ESD,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
  ...
  END DO
     ...
     DO i = ista,iend ! initiating c1 and c2
       DO j = 1,ny
         C1(j,i) = 0.0d0
         C2(j,i) = 0.0d0
       END DO
  ...
  !
    ...
    IF (InC.eq.1) THEN ! From Fortran
      ...
      ! Make sure odd modes are zero if needed
      IF (ESD.eq.0) THEN
        DO i = ista,iend
          IF (mod(i,2).eq.0) THEN ! i is even, ie it is and odd number
              jmin = 2 ! So we wawnt j to be even
          ELSE
            jmin = 1
          END IF
          DO j = jmin,ny,2
    ...
    ELSE IF (InC.eq.2) THEN ! Few modes approx
      DO i = ista,iend
      DO j = 1,ny
      ...
      ELSE IF (MOD(i+j,2).eq.1.and.kn2(j,i).le.kmax) THEN ! Adding some even modes on top
        ...
        ps(j,i) = real(nx,kind=GP)*real(ny,kind=GP)*1.0d0*1e-3*(cos(rannum)+im*sin(rannum))
        theta(j,i) = real(nx,kind=GP)*real(ny,kind=GP)*1.0d0*1e-3*(cos(rannum)-im*sin(rannum))
      ELSE
        ps(j,i) = 0.0d0
        theta(j,i) = 0.0d0
      END IF
   ...
   ELSE IF (InC.eq.3) THEN ! Matlab
     ALLOCATE( Rpsi(40000), Ipsi(40000), Rtheta(40000), Itheta(40000) ) ! Allocation memory to get the ICs, length of the IC vector in r+ij form
     ...
     DO i = ista,iend ! Setting to 0 and then adding ICs
       DO j = 1,ny
         ps(j,i) = 0.0d0
         theta(j,i) = 0.0d0
         IF (MOD(i+j,2).eq.1.and.i.lt.201.and.j.lt.401) THEN ! Checking that mode is even and that we are within the bounds of PsiE and ThetaE
           ps(j,i) = real(Rpsi((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP) ! The number here is just  N/4
           theta(j,i) = real(Rtheta((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
           IF (i.ne.1) THEN ! If we have an imaginary part also
             ps(j,i) = ps(j,i) + im*real(Ipsi((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
             theta(j,i) = theta(j,i) + im*real(Itheta((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)
           END IF
         END IF
    ...
    END IF ! end if for ics


    ! Making sure dealiasing modes are zero
    DO i = ista, iend
      DO j = 1,ny
        IF (kn2(j,i).gt.kmax) THEN
          ps(j,i) = 0.0d0
          theta(j,i) = 0.0d0
        END IF
      END DO
   ...
   ! Now we solve the even equations
   timec = cstep ! So that we have a print straigth away
   DO WHILE (abs(delta).gt.convlim.and.ESD.eq.0)        !
     ...
     ! Copies the streamfunction and theta into the auxiliary matrix C1 and C2
        DO i = ista,iend
          IF (mod(i,2).eq.0) THEN ! i is even, ie it is an odd number
              jmin = 1 ! So we wawnt j to be odd
          ELSE
            jmin = 2
          END IF
          DO j = jmin,ny,2
     ...
     ! Runge-Kutta step 2
      ...
      DO o = ord,2,-1
          ...
          DO i = ista,iend
            ...
            END IF
            DO j = jmin,ny,2
              IF (kn2(j,i).le.kmax) THEN ! We only want even modes
    ...
    DO i = ista,iend ! adding the pert
      DO j = 1,ny
        IF (MOD(i+j,2).eq.0.and.i.lt.201.and.j.lt.401) THEN ! Checking that mode is odd and that we are within the bounds of PsiV and ThetaV
          ps(j,i) = real(Rpsi((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts ! The number here is just N/4
          theta(j,i) = real(Rtheta((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts
          IF (i.ne.1) THEN ! If we have an imaginary part also
            ps(j,i) = ps(j,i) + im*real(Ipsi((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts
            theta(j,i) = theta(j,i) + im*real(Itheta((j-1)*100+1+(i-1-mod(i-1,2))/2),kind=GP)*real(nx,kind=GP)*real(ny,kind=GP)*perts
          END IF
        END IF
    ...
    !! Output times
    if (stat.le.1) then ! Phil: If stat is 0 or 1, we start from scratch.
      time = 0.0d0
    ...
    DO i = ista,iend ! initlizing c1 and c2
      DO j = 1,ny
        C1(j,i) = ps(j,i)
        C2(j,i) = theta(j,i)
      END DO
    END DO
    CALL CFL_condition(ps,nu,kappa,CFL,ord,ordvf,dt) ! Check CFL condition onle once
    !#################### MAIN LOOP ###################### ! This solve the odd system
    RK : DO t = ini,step
      ...
      DO i = ista,iend
        IF (mod(i,2).eq.0) THEN ! i is even, ie it is an odd number
            jmin = 2 ! So we want j to be even
        ELSE
          jmin = 1
        END IF
        DO j = jmin,ny,2
