...
!     ldir : local directory for I/O
      ...
      CALL MPI_BCAST( tdir,100,MPI_CHARACTER,       0,MPI_COMM_WORLD,ierr)
        ...
        ! Calculating nu
  nu = (2*pi)**(4*ordvf-1)/Ra ! Phil: Think hight (and width) of domain in 2\pi. If this is the case, then we can have integer wavenumbers in both x and y
...
! mhdcheck subroutine for details.
          IF (timec.eq.cstep) THEN
              timec = 0
              CALL hdcheck(ps,time,ordvf,ordvh)
          ENDIF
...
! with the power spectrum
         IF (times.eq.sstep) THEN
            ...
            CALL dubderivk2(ps,C1,1)       ! psi_xx
            CALL vectrans2(ps,C1,-ordvf,'fenktrans',ext4) ! Temperature Gradient Forcing
...
! Runge-Kutta step 2
         ...
         DO i = ista,iend
            DO j = 1,n
            IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
              C1(j,i) = (ps(j,i) + dt*( C3(j,i)/ka2(j,i)+(-1)**(ordvf+1)*((ka(i)**2)/(2*pi))*C1(j,i)/ka2(j,i)**(ordvf+1))/dble(o)) &
...
! Copies the result from the auxiliary matrixes into ps, az
         ...
         DO i = ista,iend
            DO j = 1,n
            IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
              ps(j,i) = (ps(j,i)+ dt*(C3(j,i)/ka2(j,i)+(-1)**(ordvf+1)*((ka(i)**2)/(2*pi))*C1(j,i)/ka2(j,i)**(ordvf+1))/dble(o)) &
              /(1.0d0 + (hnu/ka2(j,i)**ordvh+nu*ka2(j,i)**ordvf)*dt/dble(o))
