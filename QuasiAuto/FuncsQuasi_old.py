...
def ICpathMaker(path,SoN,PrS,RaS):
    if SoN == "S":
        path = path + PrS + '/Shearing/'
    else:
        path =  path + PrS + '/NonShearing/'
...
def MakeRun(compname,path,paramspath,ICPathbase,thresh,cost,CodePath,oone,maxrun,nmpi):
    # Loading in the params file and picking out params
    f=open(paramspath,"r")
    lines=f.readlines()
    f.close()
    # Checking if we have done all we need and then close job
    isFull  =  len(lines[-1]) > 20
    if isFull:
        print('params.txt is full')
        print('')
        return
    """# Check if process is running slow
    check, slowq = GetCPU()
    if not(slowq): # There is a process running slow
        print('There is a processs running at ' + str(check) + '%')
        print('')
        return"""
    """# Check if we have too many running
    maxrun = int(thresh/cost)
    running = GetNumberRuns()
    if running >= maxrun:
        print('There are already ' + str(running) + ' runs running')
        print('')
        return"""
    running = GetNumberRuns()
    print('Currently there are ' + str(running) + ' runs running')
    """# Check how many empty cores there are
    empt = EmptyCores()
    print('There are ' + str(empt) + ' empty cores')"""
    ...
    if load < thresh - cost and running < maxrun: #and empt > 7:
        if oone: # we only want one run
            numRun = 1
        else:
            #numRun = min(math.floor((thresh-load)/cost),math.floor(empt/8)) # First thing checks how many we could add pased on total CPU usade. It wont be negative due to if statment above. Second is how many was can add based on the number of "empty" cpu cores
            numRun = min(math.floor((thresh-load)/cost),maxrun-running)
