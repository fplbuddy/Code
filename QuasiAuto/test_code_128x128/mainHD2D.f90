...
PROGRAM HD2D
  ...
  COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: ps2d, ps3d
  COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: theta2d, theta3d
  !
  ...
  REAL(KIND=GP) :: theta2d0, u0, ps3d0, theta3d0
  REAL(KIND=GP) :: time
  REAL(KIND=GP) :: rannum
  !REAL(KIND=GP) :: cort
  REAL(KIND=GP) :: nu,kappa, Ra, Pr, Ly
  ...
  ALLOCATE( theta2d(ny,ista:iend) )
  ALLOCATE( ps2d(ny,ista:iend) )
  ALLOCATE( ps3d(ny,ista:iend) )
  ALLOCATE( theta3d(ny,ista:iend) )
  ...
  CALL MPI_BCAST(theta3d0, 1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  CALL MPI_BCAST(u0, 1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  CALL MPI_BCAST(theta2d0, 1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  CALL MPI_BCAST(ps3d0, 1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  ...
  nu = sqrt((pi*Lz)**3*Pr/Ra)
  kappa = sqrt((pi*Lz)**3/(Ra*Pr))
  ...
  specmax = int(sqrt(2.0d0)*max(nx,2*ny)/(3*min(Lx,Lz))) + 1 ! largest value NINT(sqrt(kk2(j,i))) can take
  ...
  IF (stat.eq.0) THEN
      ...
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN
            ...
            ps2d(j,i) = real(nx,kind=GP)*real(ny,kind=GP)*exp(rannum*im)/kk2(j,i) ! absolulte value is 1 in all of the modes
            rannum = 2*pi*randu(seed)
            theta2d(j,i) = real(nx,kind=GP)*real(ny,kind=GP)*exp(rannum*im)
            rannum = 2*pi*randu(seed)
            ps3d(j,i) = real(nx,kind=GP)*real(ny,kind=GP)*exp(rannum*im)/kk2(j,i) ! absolulte value is 1 in all of the modes
            rannum = 2*pi*randu(seed)
            theta3d(j,i) = real(nx,kind=GP)*real(ny,kind=GP)*exp(rannum*im)
          ELSE
            ps2d(j,i) = 0.0d0
            theta2d(j,i) = 0.0d0
            ps3d(j,i) = 0.0d0
    ...
    ELSE IF (InC.eq.2) THEN
      ALLOCATE( Rps2d(size), Ips2d(size), Rtheta2d(size), Itheta2d(size), Rps3d(size), Ips3d(size), Rtheta3d(size), Itheta3d(size) )
      IF (myrank.eq.0) THEN
        OPEN(1,file='ps2d.txt',status='unknown') ! Getting ps ICs
        do i = 1, size, 1
          read(1,*) Rps2d(i)
          read(1,*) Ips2d(i)
        end do
        CLOSE(1)
        OPEN(1,file='theta2d.txt',status='unknown') ! Getting theta2d ICs
        do i = 1, size, 1
          read(1,*) Rtheta2d(i)
          read(1,*) Itheta2d(i)
        end do
        CLOSE(1)
        OPEN(1,file='ps3d.txt',status='unknown') ! Getting theta2d ICs
        do i = 1, size, 1
          read(1,*) Rps3d(i)
          read(1,*) Ips3d(i)
        end do
        CLOSE(1)
        OPEN(1,file='theta3d.txt',status='unknown') ! Getting theta2d ICs
        do i = 1, size, 1
          read(1,*) Rtheta3d(i)
          read(1,*) Itheta3d(i)
        end do
      ...
      DO i = ista,iend ! Setting to 0 and then adding ICs
        DO j = 1,ny
          ...
          IF (i.lt.(nx/2+1).and.j.lt.(ny+1)) THEN ! Checking that mode is even and that we are within the bounds of psE and ThetaE
            ps2d(j,i) = real(Rps2d((j-1)*(nx/2)+i),kind=GP)*tmp
            theta2d(j,i) = real(Rtheta2d((j-1)*(nx/2)+i),kind=GP)*tmp
            ps3d(j,i) = real(Rps3d((j-1)*(nx/2)+i),kind=GP)*tmp
            theta3d(j,i) = real(Rtheta3d((j-1)*(nx/2)+i),kind=GP)*tmp
            IF (i.ne.1) THEN ! If we have an imaginary part also
              ps2d(j,i) = ps2d(j,i) + im*real(Ips2d((j-1)*(nx/2)+i),kind=GP)*tmp
              theta2d(j,i) = theta2d(j,i) + im*real(Itheta2d((j-1)*(nx/2)+i),kind=GP)*tmp
              ps3d(j,i) = ps2d(j,i) + im*real(Ips3d((j-1)*(nx/2)+i),kind=GP)*tmp
              theta3d(j,i) = theta2d(j,i) + im*real(Itheta3d((j-1)*(nx/2)+i),kind=GP)*tmp
            END IF
          END IF
      ...
      DEALLOCATE( Rps2d, Ips2d, Rtheta2d, Itheta2d, Rps3d, Ips3d, Rtheta3d, Itheta3d )

    ELSE IF (InC.eq.3) THEN ! random for 3d fields, matlab for 2d
      ALLOCATE( Rps2d(size), Ips2d(size), Rtheta2d(size), Itheta2d(size) )
      IF (myrank.eq.0) THEN
        OPEN(1,file='ps2d.txt',status='unknown') ! Getting ps ICs
        do i = 1, size, 1
          read(1,*) Rps2d(i)
      ...
      DO i = ista,iend ! Setting to 0 and then adding ICs
        ...
        END DO
      END DO
      DEALLOCATE( Rps2d, Ips2d, Rtheta2d, Itheta2d )
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN
            rannum = 2*pi*randu(seed)
        ...
        DO i = ista,iend
          DO j = 1,ny
            IF (kn2(j,i).le.kmax) THEN ! Don't actually think we need this here since alising only comes into play when non-linear terms are used
              theta2d(j,i) = tmp*theta2d(j,i)
            ELSE
              theta2d(j,i) = 0.0d0 ! Gets rid of the wavenumbers that are too large
            END IF
          END DO
        END DO
      ! psi
      CALL energy(ps2d,enerk,1)
      CALL MPI_BCAST(enerk,1,GC_REAL,0,MPI_COMM_WORLD,ierr)
      tmp=u0/sqrt(enerk)
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN ! Don't actually think we need this here since alising only comes into play when non-linear terms are used
            ps2d(j,i) = tmp*ps2d(j,i)
          ELSE
            ps2d(j,i) = 0.0d0 ! Gets rid of the wavenumbers that are too large
          END IF
        END DO
      END DO
    ...
    IF (normtwo.eq.1) THEN ! Dont want to normalise when ICs is from MATLAB
      CALL energy(theta3d,enerk,0)
      CALL MPI_BCAST(enerk,1,GC_REAL,0,MPI_COMM_WORLD,ierr)
      tmp=theta3d0/sqrt(enerk)
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN ! Don't actually think we need this here since alising only comes into play when non-linear terms are used
            theta3d(j,i) = tmp*theta3d(j,i)
          ELSE
            theta3d(j,i) = 0.0d0 ! Gets rid of the wavenumbers that are too large
          END IF
        END DO
      END DO
      ! ps3d
      CALL energy(ps3d,enerk,1)
      CALL MPI_BCAST(enerk,1,GC_REAL,0,MPI_COMM_WORLD,ierr)
      tmp=ps3d0/sqrt(enerk)
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN ! Don't actually think we need this here since alising only comes into play when non-linear terms are used
            ps3d(j,i) = tmp*ps3d(j,i)
          ELSE
            ps3d(j,i) = 0.0d0 ! Gets rid of the wavenumbers that are too large
          END IF
        END DO
      END DO
  ...
  ELSE  !! stat
    ...
    OPEN(1,file=trim(ldir) // '/hd2Dps2d.' // node // '.' &
    // x // c // d // u //'.dat',form='unformatted')
    READ(1) R1
    CLOSE(1)
    CALL fftp2d_real_to_complex(planrc,R1,ps2d,MPI_COMM_WORLD)
    ...
    DO j = jsta,jend
      ...
      END DO
    END DO
    OPEN(1,file=trim(ldir) // '/hd2Dtheta2d.' // node // '.' &
    // x // c // d // u //'.dat',form='unformatted')
    READ(1) R1
    CLOSE(1)
    CALL fftp2d_real_to_complex(planrc,R1,theta2d,MPI_COMM_WORLD)
    ...
    DO j = jsta,jend
      ...
      END DO
    END DO
    OPEN(1,file=trim(ldir) // '/hd2Dps3d.' // node // '.' &
    // x // c // d // u //'.dat',form='unformatted')
    READ(1) R1
    CLOSE(1)
    CALL fftp2d_real_to_complex(planrc,R1,ps3d,MPI_COMM_WORLD)
    ...
    DO j = jsta,jend
      ...
      END DO
    END DO
    OPEN(1,file=trim(ldir) // '/hd2Dtheta3d.' // node // '.' &
    // x // c // d // u //'.dat',form='unformatted')
    READ(1) R1
    CLOSE(1)
    CALL fftp2d_real_to_complex(planrc,R1,theta3d,MPI_COMM_WORLD)
  ...
  !! Output times
  if (stat.eq.0) then ! ps3dil: If stat is 0, we will get output straig away basically
    time = 0.0d0
  ...
  !
    ...
    CALL CFL_condition(ps2d,nu,kappa,CFL,ord,ordvf,Lx,Lz,dt)
    !
    ...
    IF (timec.eq.cstep) THEN
      CALL hdcheck(ps2d,theta2d,ps3d,theta3d,time,ordvf,ordvh)
      timec = 0
    ENDIF
    ...
    IF (times.eq.sstep) THEN
      ...
      CALL spectrum(ps2d,theta2d,ext4,'spec2d')
      CALL spectrum(ps3d,theta3d,ext4,'spec3d')
      IF (myrank.eq.0) THEN
    ...
    !
    ! Every 'tstep' steps2d, stores the results of the integration
    ...
    !
    ! Runge-Kutta step 1
    ! Copies the streamfunction and theta2d into the auxiliary matrix C1 and C2
    ...
    DO o = ord,2,-1
      CALL poisson(C1,C2,C4)  ! messed up notes, see v1.
      CALL laplak2(C1,C3)
      CALL poisson(C5,C3,C7)
      CALL poisson(C1,C3,C3)
      CALL laplak2(C5,C8)
      CALL poisson(C1,C8,C8)
      CALL poisson(C1,C6,C9)
      CALL poisson(C5,C2,C10)

      rmp = 1.0_GP/real(o,kind=GP)
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN
            C1(j,i) = (ps2d(j,i) + dt*rmp*( C3(j,i)/kk2(j,i)-im*kx(i)*C2(j,i)/kk2(j,i))) &
            /(1.0d0 + (nu*kk2(j,i))*dt*rmp)
            C2(j,i) = (theta2d(j,i) + dt*rmp*(-C4(j,i)+im*kx(i)*C1(j,i)/pi)) &
            /(1.0d0 + (kappa*kk2(j,i))*dt*rmp)

            C5(j,i) = (ps3d(j,i) + dt*rmp*( C7(j,i)/kk2(j,i)+C8(j,i)/kk2(j,i)-im*kx(i)*C6(j,i)/kk2(j,i))) &
            /(1.0d0 + (nu*(kk2(j,i)+(2.0d0*pi/Ly)**2))*dt*rmp)

            C6(j,i) = (theta3d(j,i) + dt*rmp*( -C9(j,i)-C10(j,i)+im*kx(i)*C5(j,i)/pi)) &
            /(1.0d0 + (kappa*(kk2(j,i)+(2.0d0*pi/Ly)**2))*dt*rmp)
          ELSE
            C1(j,i) = 0.0d0
            C2(j,i) = 0.0d0
            C5(j,i) = 0.0d0
    ...
    DO i = ista,iend
      DO j = 1,ny
        IF (kn2(j,i).le.kmax) THEN
          ps2d(j,i) = (ps2d(j,i)+ dt*rmp*( C3(j,i)/kk2(j,i)-im*kx(i)*C2(j,i)/kk2(j,i))) &
          /(1.0d0 + (nu*kk2(j,i))*dt*rmp)

          theta2d(j,i) = (theta2d(j,i)+ dt*rmp*(-C4(j,i)+im*kx(i)*C1(j,i)/pi)) &
          /(1.0d0 + (kappa*kk2(j,i))*dt*rmp)

          ps3d(j,i) = (ps3d(j,i) + dt*rmp*( C7(j,i)/kk2(j,i)+C8(j,i)/kk2(j,i)-im*kx(i)*C6(j,i)/kk2(j,i))) &
          /(1.0d0 + (nu*(kk2(j,i)+(2.0d0*pi/Ly)**2))*dt*rmp)

          theta3d(j,i) = (theta3d(j,i) + dt*rmp*( -C9(j,i)-C10(j,i)+im*kx(i)*C5(j,i)/pi)) &
          /(1.0d0 + (kappa*(kk2(j,i)+(2.0d0*pi/Ly)**2))*dt*rmp)
        ELSE
          ps2d(j,i) = 0.0d0
          theta2d(j,i) = 0.0d0
  ...
  DEALLOCATE( R1,R2 )
  DEALLOCATE( ps2d,theta2d, theta3d, ps3d )
