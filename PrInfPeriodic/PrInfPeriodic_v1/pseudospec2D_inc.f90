...
!*****************************************************************
      SUBROUTINE hdcheck(a,b,t,ordvf)
!-----------------------------------------------------------------
!
...
!
      ...
      DOUBLE COMPLEX, DIMENSION(n,ista:iend) :: a,b,c1
...
!
      ...
      CALL derivk2(a,c1,1) ! ps_x
      CALL inerprod(c1,b,0,fenp)
...
!
   ...
   20    FORMAT( E23.14E3 )
      ...
      !     a  : theta
      !     hnu: hyper visc
      !     ordvf: normal visc power
      !     ordvh: hyper visc power
      !     b : (out) ps
      !
            USE ali
            USE kes
            ...
            RETURN
          END SUBROUTINE makeps

!*****************************************************************
      SUBROUTINE CFL_condition(c1,kappa,cfl,ord,ordvf,dt)
!-----------------------------------------------------------------
!
...
!      b0 : external
      ...
      if (ord.eq.2) then
         dt = cfl*min((sqrt(3.0d0)/pi)*dx/umax, &
                       (2.0d0/pi**2)*dx**(2*ordvf)/kappa)
      else if (ord.eq.3) then
         dt = cfl*min((sqrt(3.0d0)/pi)*dx/umax, &
                      (2.5d0/pi**2)*dx**(2*ordvf)/kappa)
      end if
