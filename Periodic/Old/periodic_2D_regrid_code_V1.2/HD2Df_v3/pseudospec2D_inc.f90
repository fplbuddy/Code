...
!*****************************************************************
SUBROUTINE hdcheck(a,b,t,ordvf,ordvh)
  !-----------------------------------------------------------------
  !
  ...
  DOUBLE COMPLEX, DIMENSION(n,ista:iend) :: a,b
  DOUBLE PRECISION  :: enk,denk
  DOUBLE PRECISION  :: ens,dens
  DOUBLE PRECISION  :: t
...
SUBROUTINE vectrans(a,b,c,ext1,ext2)
  ...
  MPI_COMM_WORLD,ierr)
  IF (myrank.eq.0) THEN
    OPEN(1,file=trim(tdir) // '/vectrans_' // ext1 // '.' // ext2 // '.txt')
    WRITE(1,20) Ektot
    20    FORMAT( E23.15 )
    CLOSE(1)
  ENDIF
