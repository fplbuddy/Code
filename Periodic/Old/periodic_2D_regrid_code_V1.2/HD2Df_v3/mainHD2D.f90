...
PROGRAM HD2D
  ...
  DOUBLE COMPLEX, ALLOCATABLE, DIMENSION (:,:) :: f
  !
  ...
  DOUBLE PRECISION :: u0, theta0,f0
  DOUBLE PRECISION :: time,timef
  DOUBLE PRECISION :: cort
  DOUBLE PRECISION :: nu,kappa,Ra,Pr,d,Rh
  ...
  CALL MPI_BCAST(   Rh,  1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
  ...
  nu = sqrt((2*pi)**(4*ordvf-1)*Pr/Ra) ! Phil: Think hight (and width) of domain in 2\pi. If this is the case, then we can have integer wavenumbers in both x and y
  kappa = sqrt((2*pi)**(4*ordvf-1)/(Ra*Pr))
  d = Rh*sqrt((2*pi)**(4*ordvf-1)/(Ra*Pr))/((2*pi)**(2*ordvf+2))
  ...
  DO i = 1,n/2
    ka(i) = dble(i-1) ! Phil: ka = (0, 1,...,n/2-1,-n/2,-n/2+1,...,-1), So again have this issue where don't have c.c. for -n/2. But I guess Allising fixes it
  ...
  ! IF (myrank.eq.0) stop ! Phil: Don't know why we have stoped proc 0?

  ! Make Forcing
  !DO j = jsta,jend
  !   DO i = 1,n
  !      R1(i,j) = 0.0d0 ! resetting the real matrix just in case
  !   END DO
  !END DO

  !DO j = jsta,jend
  !   DO i = 1,n
  !      R1(i,j) = R1(i,j) + cos(4*pi*(dble(i)-1)/dble(n)) &
  !                        * cos(4*pi*(dble(j)-1)/dble(n))
  !   END DO
  !END DO
  !CALL fftp2d_real_to_complex(planrc,R1,f,MPI_COMM_WORLD)

  ! Normalise f
  !CALL energy(f,enerk,0)
  !call MPI_BCAST(enerk,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
  !tmp1=sqrt(f0/(enerk*4.0d0))
  !DO i = ista,iend
  !     DO j = 1,n
  ...
  !     END DO
  !  END DO
  tmp1 = f0*dble(n)**2/4.0d0
  DO i = ista,iend
    DO j = 1,n
      IF ((j.eq.3.or.j.eq.(n-1)).and.(i.eq.3)) THEN ! putting it into (2,2) and (2,-2) modes
        f(j,i) = tmp1
      ELSE
        f(j,i) = 0.0d0
      ENDIF
  ...
  IF (stat.eq.0) THEN
    ...
    if (iflow.eq.1) then
      ...
      DO i = ista,iend
        DO j = 1,n
          IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
            theta(j,i) = im*ps(j,i) ! Set them up to differ by a phase
          ELSE
            theta(j,i) = 0.0d0
          ENDIF
  ...
  !#################### MAIN LOOP ######################
  RK : DO t = ini,step
    CALL CFL_condition(ps,nu,kappa,d,CFL,ord,ordvf,dt)
    !      print*,t
    ...
    IF (times.eq.sstep) THEN
      ...
      OPEN(1,file=trim(sdir) // '/ps.' // node // '.' &
      // ext // '.txt',position='append')
      DO i = ista,iend
        DO j = 1,n
          IF (ka2(j,i).le.kmax) THEN
            WRITE(1,20) REAL(ps(j,i))*tmp1, IMAG(ps(j,i))*tmp1, i, j
            20    FORMAT( E23.14E3, E23.14E3, I3, I3 )
          END IF
        END DO
      END DO
      CLOSE(1)
      OPEN(1,file=trim(sdir) // '/theta.' // node // '.' &
      // ext // '.txt',position='append')
      DO i = ista,iend
        DO j = 1,n
          IF (ka2(j,i).le.kmax) THEN
            WRITE(1,20) REAL(theta(j,i))*tmp1, IMAG(theta(j,i))*tmp1, i, j
          END IF
        END DO
      END DO
      CLOSE(1)
      CALL spectrum(ps,theta,ext4)
      IF (myrank.eq.0) THEN
      ...
      DO i = ista,iend
        DO j = 1,n
          IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
            ...
            /(1.0d0 + (d/ka2(j,i)+nu*ka2(j,i)**ordvf)*dt/dble(o))

            C2(j,i) = (theta(j,i) + dt*(-C4(j,i)+im*ka(i)*C1(j,i)/(2*pi)+f(j,i))/dble(o)) &
    ...
    DO i = ista,iend
      DO j = 1,n
        IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
          ...
          theta(j,i) = (theta(j,i)+ dt*(-C4(j,i)+im*ka(i)*C1(j,i)/(2*pi)+f(j,i))/dble(o)) &
          /(1.0d0 + (kappa*ka2(j,i)**ordvf)*dt/dble(o))
        ELSE
          ps(j,i) = 0.0d0
          theta(j,i) = 0.0d0
