...
PROGRAM HD2D
  ...
  DOUBLE PRECISION :: nu,kappa,mu, DT
  ...
  RK : DO t = ini,step
    ...
    IF (timec.eq.cstep) THEN
      timec = 0
      CALL hdcheck(ps,theta,f,time,ordvf,ordvh)
    ENDIF
    ...
    IF (times.eq.sstep) THEN
      ...
      CALL vectrans(theta,ps,theta,'puu',ext4)
      ...
      DO i = ista,iend
        DO j = 1,n
          IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
            ...
            /(1.0d0 + (mu/ka2(j,i)**ordvh+nu*ka2(j,i)**ordvf)*dt/dble(o))

            C2(j,i) = (theta(j,i) + dt*(-C4(j,i)+DT*im*ka(i)*C1(j,i)/(2*pi)+f(j,i))/dble(o)) &
    ...
    DO i = ista,iend
      DO j = 1,n
        IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
          ...
          theta(j,i) = (theta(j,i)+ dt*(-C4(j,i)+DT*im*ka(i)*C1(j,i)/(2*pi)+f(j,i))/dble(o)) &
          /(1.0d0 + (kappa*ka2(j,i)**ordvf)*dt/dble(o))
        ELSE
          ps(j,i) = 0.0d0
          theta(j,i) = 0.0d0
