...
!*****************************************************************
SUBROUTINE hdcheck(a,b,f,t,ordvf,ordvh)
  !-----------------------------------------------------------------
  !
  ...
  DOUBLE COMPLEX, DIMENSION(n,ista:iend) :: a,b,f
  DOUBLE COMPLEX, DIMENSION(n,ista:iend)  :: c1
  DOUBLE PRECISION  :: enk,denk, vth,henk
  DOUBLE PRECISION  :: enp, denp, fin
  DOUBLE PRECISION  :: ens,dens,hens
  DOUBLE PRECISION  :: t
  ...
  CALL inerprod(c1,b,0,vth)  ! integrates v*tt

  CALL energy(b,enp,0)       ! integrates tt^2
  CALL energy(b,denp,ordvf)
  CALL inerprod(b,f,0,fin)  ! integrates tt*f
  ...
  IF (myrank.eq.0) THEN
    ...
    AIMAG(a(1,2))*nrm, &
    AIMAG(a(1,3))*nrm
    40    FORMAT( E23.14E3,E23.14E3,E23.14E3,E23.14E3,E23.14E3,E23.14E3, E23.14E3)
    CLOSE(1)
...
SUBROUTINE mhdcheck(a,b,c,d,t,eng,kup,ordvf,ordvh)
  ...
  IF (myrank.eq.0) THEN
    ...
    10    FORMAT( E22.14,E22.14,E22.14,E22.14)
    !   10    FORMAT( E22.14,E22.14,E22.14,E22.14,E22.14)
    CLOSE(1)
    ...
    !!$   11    FORMAT( E22.14,E22.14,E22.14,E22.14,E22.14)
...
SUBROUTINE spectrum(a,b,ext)
  ...
  DOUBLE PRECISION, DIMENSION(n/2+1)        :: Ep,Eptot
  DOUBLE COMPLEX, DIMENSION(n,ista:iend)    :: a,b
  ...
  MPI_COMM_WORLD,ierr)
  CALL MPI_REDUCE(Ep,Eptot,n/2+1,MPI_DOUBLE_PRECISION,MPI_SUM,0, &
  MPI_COMM_WORLD,ierr)
  IF (myrank.eq.0) THEN
...
!*****************************************************************
!SUBROUTINE fieldsfs(a,b,ext)
  !-----------------------------------------------------------------
  !
  ! Outputs all the modes in ps and theta into .txt files
  !
  ! Parameters
  !     a  : streamfunction
...
!        WRITE(1,20) REAL(a(j,i))*nrm, IMAG(a(j,i))*nrm, i, j
...
!    DO j = 1,n
!      IF (ka2(j,i).le.kmax) THEN
!        WRITE(1,20) REAL(b(j,i))*nrm, IMAG(b(j,i))*nrm, i, j
!        20    FORMAT( E23.14E3, E23.14E3, I3, I3 )
!      END IF
!    END DO
!  END DO
...
!*****************************************************************
SUBROUTINE CFL_condition(c1,nu,kappa,d,cfl,ord,vord,dt)
  !-----------------------------------------------------------------
  !
