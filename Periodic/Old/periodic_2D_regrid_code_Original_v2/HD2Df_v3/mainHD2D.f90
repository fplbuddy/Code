...
!FORCING
      ...
      DO j = jsta,jend
         DO i = 1,n
            R1(i,j) = sin(2*pi*kfup*(dble(i)-1)/dble(n)) &
                    * sin(2*pi*kfup*(dble(j)-1)/dble(n))
         END DO
      END DO
      ...
      do ir = kfup,kfdn
         phase1 = 0.!pi*randu(seed)
         phase2 = 0.!pi*randu(seed)
         do j = jsta,jend
            do i = 1,n

            !wz with ux =  sinx + siny & uy = -sinx + siny
            !R1(i,j) = R1(i,j)-cos(2.*pi*ir*(dble(i)-1)/dble(n)+phase1) &
            !                 -cos(2.*pi*ir*(dble(j)-1)/dble(n)+phase2)
            R1(i,j) = cos(2*pi*kfup*(dble(i)-1)/dble(n)) &
                     *cos(2*pi*kfup*(dble(j)-1)/dble(n))

!!!            !jz with bx =  sinx + siny & by = -siny + sinx
!!!            R2(i,j) = R2(i,j)+cos(2.*pi*ir*(dble(i)-1)/dble(n)+phase1) &
!!!                             -cos(2.*pi*ir*(dble(j)-1)/dble(n)+phase2)
      ...
      CALL energy(fk,enerk,0)
      call MPI_BCAST(enerk,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
      tmp1=(f0/2.0d0)*sqrt(1.0d0/enerk)
      DO i = ista,iend
         DO j = 1,n
            IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
               !fk(j,i) = tmp1*ps(j,i)/ka2(j,i)
               fk(j,i) = tmp1*fk(j,i)
            ELSE
               fk(j,i) = 0.0d0
            ENDIF
...
!STREAM FUNCTION R1 & VECTOR POTENTIAL R2
      ...
      do ir = kup,kdn
         ...
         do j = jsta,jend
            ...
            !wz with ux =  sinx + siny & uy = -sinx + siny
            R1(i,j) = R1(i,j)-cos(2.0d0*pi*ir*(dble(i)-1)/dble(n)+phase1) &
                             -cos(2.0d0*pi*ir*(dble(j)-1)/dble(n)+phase2)
...
!!!            !jz with bx =  sinx + siny & by = -siny + sinx
!!!            R2(i,j) = R2(i,j)+cos(2.0d0*pi*ir*(dble(i)-1)/dble(n)+phase1) &
!!!                             -cos(2.0d0*pi*ir*(dble(j)-1)/dble(n)+phase2)
      ...
      DO j = jsta,jend
         DO i = 1,n
         R1(i,j) = sin( 1.3*pi*sin(2*kup*pi*(dble(i)-1)/dble(n)) ) &
                 * (1d0-cos(2*pi*(dble(j)-1)/dble(n)))**4          &
                 *      sin(2*pi*(dble(i)-1)/dble(n))**6
!!!         R2(i,j) = R1(i,j)*sin(2*pi*(dble(i)-1)/dble(n))
         END DO
      END DO
      ...
      DO j = jsta,jend
         DO i = 1,n
      R1(i,j)= R1(i,j) &
             + cos(2.0d0*ir *pi*(dble(i)-1)/dble(n)+phase1)/ir*prm1 &
             + cos(2.0d0*ir *pi*(dble(j)-1)/dble(n)+phase2)/ir*prm1
!!!      R2(i,j)= R2(i,j) &
!!!             + cos(2.0d0*ir *pi*(dble(i)-1)/dble(n)+phase3)/ir*prm1 &
!!!             + cos(2.0d0*ir *pi*(dble(j)-1)/dble(n)+phase4)/ir*prm1
         END DO
      END DO
      ENDDO
      DO ir = 1,5
      ...
      DO j = jsta,jend
         DO i = 1,n
            R1(i,j) = R1(i,j) &
                    + cos(2.0d0*ir *pi*(dble(i)-1)/dble(n)+phase1) &
                    * cos(2.0d0*jr *pi*(dble(j)-1)/dble(n)+phase2)/kr
!!!            R2(i,j) = R2(i,j) &
!!!                    + cos(2.0d0*ir *pi*(dble(i)-1)/dble(n)+phase3) &
!!!                    * cos(2.0d0*jr *pi*(dble(j)-1)/dble(n)+phase4)/kr
         END DO
      END DO
      END DO
      END DO
...
!#################### MAIN LOOP ######################
 RK : DO t = ini,step
      CALL CFL_condition(ps,nu,CFL,ord,ordvf,dt)
!      print*,t
...
! mhdcheck subroutine for details.
          IF (timec.eq.cstep) THEN
              timec = 0
              CALL hdcheck(ps,fk,time,ordvf,ordvh)
          ENDIF
...
! with the power spectrum
         IF (times.eq.sstep) THEN
            ...
            CALL vectrans2(ps,ps,1+ordvf,'denktrans',ext4)
            CALL vectrans2(ps,f,1,'fenktrans',ext4)
            CALL vectrans2(ps,ps,1-ordvh,'henktrans',ext4)
           IF (myrank.eq.0) THEN
...
! Runge-Kutta step 2
         ...
         CALL poisson(C1,C3,C1)  ! u grad w

         DO i = ista,iend
            DO j = 1,n
            IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
               C1(j,i) = (ps(j,i)+ dt*( - C1(j,i)/ka2(j,i)  &
                                        + fk(j,i))/dble(o)) &
...
! Copies the result from the auxiliary matrixes into ps, az
         ...
         DO i = ista,iend
            DO j = 1,n
            IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
               ps(j,i) = (ps(j,i)+ dt*( - C1(j,i)/ka2(j,i)  &
                                        + fk(j,i))/dble(o)) &
               /(1.0d0 + (hnu/ka2(j,i)**ordvh+nu*ka2(j,i)**ordvf)*dt/dble(o))
            ELSE
               ps(j,i) = 0.0d0
