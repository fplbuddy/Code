...
! Some auxiliary matrixes
      ...
      DOUBLE PRECISION :: u0, theta0
      DOUBLE PRECISION :: time,timef
      DOUBLE PRECISION :: cort
      DOUBLE PRECISION :: nu,hnu,kappa,Ra,Pr
...
!     ldir : local directory for I/O
      ...
      nu = sqrt((2*pi)**3*Pr/Ra) ! Phil: Think hight (and width) of domain in 2\pi. If this is the case, then we can have integer wavenumbers in both x and y
      kappa = sqrt((2*pi)**3/(Ra*Pr))
...
! Runge-Kutta step 2
         ...
         DO i = ista,iend
            ...
            IF ((ka2(j,i).le.kmax).and.(ka2(j,i).ge.tiny)) THEN
              ...
              /(1.0d0 + (nu*ka2(j,i))*dt/dble(o))

              C2(j,i) = (theta(j,i) + dt*(-C4(j,i)+im*ka(i)*C1(j,i)/(2*pi))/dble(o)) &
              /(1.0d0 + (kappa*ka2(j,i))*dt/dble(o))
            ELSE
               C1(j,i) = 0.0d0
               C2(j,i) = 0.0d0
            ENDIF
