...
SUBROUTINE makeph(a,Wid,b)
  ...
  INTEGER :: i,j
    ...
    DO i = ista,iend
      DO j = 1,ny
        b(j,i) = -im*kx(i)*a(j,i)/((kk2(j,i) + (2.0d0*pi/Wid)**2)*kk2(j,i))
      END DO
    END DO

  RETURN
END SUBROUTINE makeph
...
!*****************************************************************
SUBROUTINE CFL_condition(a,kappa,cfl,ord,vord,Lx,Ly,dt)
  !-----------------------------------------------------------------
  !
