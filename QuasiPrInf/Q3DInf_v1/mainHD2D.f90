...
PROGRAM HD2D
  ...
  COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: C1,C2, C4, C5, C6, C9, C10
  ...
  REAL(KIND=GP) :: theta20, thetav0
  REAL(KIND=GP) :: time
  REAL(KIND=GP) :: rannum
  !REAL(KIND=GP) :: cort
  REAL(KIND=GP) :: kappa, Ra, Wid
  ...
  IF (stat.eq.0) THEN
    ...
    ELSE IF (InC.eq.2) THEN
      ALLOCATE( Rtheta2(size), Itheta2(size), Rthetav(size), Ithetav(size) )
      IF (myrank.eq.0) THEN
      ...
      DEALLOCATE( Rtheta2, Itheta2, Rthetav, Ithetav )

    ELSE IF (InC.eq.3) THEN ! random for 3d fields, matlab for 2d
      ALLOCATE( Rtheta2(size), Itheta2(size) )
      IF (myrank.eq.0) THEN
        OPEN(1,file='theta2.txt',status='unknown') ! Getting theta2 ICs
        do i = 1, size, 1
          read(1,*) Rtheta2(i)
  ...
  END IF  !! stat
    ...
    ! calculate stream functions
    CALL makeph(thetav,Wid,ph)
    CALL makeps(theta2,Wid,ps)
  ...
  !
    ...
    CALL CFL_condition(ps,kappa,CFL,ord,ordvf,Lx,Ly,dt)
    !
    ...
    DO o = ord,2,-1
      CALL makeps(C2,C1)      ! ps
      CALL makeph(C6,Wid,C5)      ! ph
    ...
    !!! MAKE PS AND PH AT END
    CALL makeps(theta2,ps)      ! ps
    CALL makeph(thetav,Wid,ph)      ! ph
  ...
  DEALLOCATE( C1,C2,C4,C5,C6,C9,C10 )
  DEALLOCATE( kx,ky )
  DEALLOCATE( kk2 )
