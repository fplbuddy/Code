...
PROGRAM HD2D
  ...
  !
    ...
    IF (times.eq.sstep) THEN
      ...
      ! also do the vectrans2D here
      !!!
      CALL laplak2(ps,C1)
      CALL poisson(ph,C1,C1)
      CALL vectrans2D(ph,C1,'MNL1',ext4)
      !!!
      CALL laplak2(ph,C1)
      CALL poisson(ps,C1,C1)
      CALL vectrans2D(ph,C1,'MNL2',ext4)
      !!!
      CALL derivk2(thetav,C1,1)
      CALL vectrans2D(ph,C1,'fenk',ext4)
      !!!
      DO i = ista,iend
        DO j = 1,ny
          C1(j,i) = (kk2(j,i)+(2.0d0*pi/Wid)**2)*kk2(j,i)*ph(j,i)
        END DO
      END DO
      CALL vectrans2D(ph,C1,'MDis',ext4)
      !!!
      CALL poisson(ps,thetav,C1)
      CALL vectrans2D(thetav,C1,'ENL1',ext4)
      !!!
      CALL poisson(ph,theta2,C1)
      CALL vectrans2D(thetav,C1,'ENL2',ext4)
      !!!
      DO i = ista,iend
        DO j = 1,ny
          C1(j,i) = -(kk2(j,i)+(2.0d0*pi/Wid)**2)*thetav(j,i)
        END DO
      END DO
      CALL vectrans2D(thetav,C1,'EDis',ext4)
