...
!*****************************************************************
SUBROUTINE vectrans2D(a,b,ext1,ext2)
  !-----------------------------------------------------------------
  !
  ! Computes the energy power spectrum in 2D.
  ! The output is written to a file by the first node.
  ...
  DOUBLE PRECISION, DIMENSION((kxmax + 1),kymax) :: E,Etot
  COMPLEX(KIND=GP), INTENT(IN), DIMENSION(ny,ista:iend) :: a, b
  DOUBLE PRECISION        :: tmp,two
  INTEGER     :: i,j
  ...
  DO i = ista,iend
    ...
    IF (i.eq.1) two=1.0d0
    DO j = 1,ny
      !kmnx = int(abs(kx(i))/Dkx+1.5d0) !int rounds dows, so 0 -> 1, 1 -> 2 here. Nor sure why we need these?
      !kmny = int(abs(ky(j))/Dky+.5d0) !Dky NOT sure
      IF (kn2(j,i).le.kmax) THEN
          E(i,j) = E(i,j)+two*dble(a(j,i)*conjg(b(j,i)))*tmp
      ENDIF
    END DO
  END DO
  CALL MPI_REDUCE(E,Etot,(kxmax + 1)*kymax,GC_REAL,MPI_SUM, &
  0,MPI_COMM_WORLD,ierr)
  IF (myrank.eq.0) THEN
    OPEN(1,file=trim(sdir) // '/' // ext1 // '.' // ext2 // '.out',form='unformatted')
    WRITE(1) Etot
    CLOSE(1)
  ENDIF

  RETURN
END SUBROUTINE vectrans2D
