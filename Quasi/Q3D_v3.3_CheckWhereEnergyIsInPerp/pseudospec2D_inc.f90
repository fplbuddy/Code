...
SUBROUTINE spectrum2D(a,b,ext)
  ...
  ! Computes the energy power spectrum in 2D.
  ! The output is written to a file by the first node.
  ! for q2d problem, I have changd it such that this works on the 3d modes
  !
  ! Parameters
  !     a  : streamfunction
  !     b  : theta
  ...
  0,MPI_COMM_WORLD,ierr)
  IF (myrank.eq.0) THEN
    OPEN(1,file=trim(sdir) // '/2Dspectrum3Dfield_KE.' // ext // '.out',form='unformatted')
    WRITE(1) Ektot
    CLOSE(1)
    OPEN(1,file=trim(sdir) // '/2Dspectrum3Dfield_PE.' // ext // '.out',form='unformatted')
    WRITE(1) Eptot
    CLOSE(1)
  ENDIF
