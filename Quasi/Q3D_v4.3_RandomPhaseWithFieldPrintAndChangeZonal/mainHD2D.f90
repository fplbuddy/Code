...
PROGRAM HD2D
  ...
  RK : DO t = ini,step
    ...
    IF (timer.eq.rstep) THEN ! An instance where we wanna do the random field
      ...
      IF (timet.eq.0) then ! if we have printed out the other field, then we print out the random fileds as well
        rmp = 1.0_GP/(2.0_GP*real(nx,kind=GP)*real(ny,kind=GP))
        ...
        OPEN(1,file=trim(ldir) // '/hd2Dpsrand.' // node // '.' &
        // ext // '.dat',form='unformatted')
        WRITE(1) R1
        CLOSE(1)
        ...
        END DO
        CALL fftp2d_complex_to_real(plancr,C3,R1,MPI_COMM_WORLD)
        OPEN(1,file=trim(ldir) // '/hd2Dtheta2rand.' // node // '.' &
        // ext // '.dat',form='unformatted')
        WRITE(1) R1
        CLOSE(1)
