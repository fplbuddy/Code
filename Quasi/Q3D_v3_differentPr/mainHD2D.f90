...
PROGRAM HD2D
  ...
  REAL(KIND=GP) :: rannum
  !REAL(KIND=GP) :: cort
  REAL(KIND=GP) :: nu1,kappa1, Ra, Pr1, Wid, Pr2, nu2, kappa2
  ...
  CALL MPI_BCAST(   Pr1,  1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  CALL MPI_BCAST(   Pr2,  1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  ...
  nu1 = sqrt((pi*Ly)**3*Pr1/Ra)
  kappa1 = sqrt((pi*Ly)**3/(Ra*Pr1))
  nu2 = sqrt((pi*Ly)**3*Pr2/Ra)
  kappa2 = sqrt((pi*Ly)**3/(Ra*Pr2))
  size = ny*nx/2 ! Think this is the right size, will have to check
  CALL MPI_BCAST(   nu1,  1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  ...
  !
    ...
    CALL CFL_condition(ps,nu1,kappa1,CFL,ord,ordvf,Lx,Ly,dt)
    !
