...
PROGRAM HD2D
  ...
  COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:) :: C1,C3,C2, C4, C5, C6, C7, C8, C9
      ...
      DO i = ista,iend
        DO j = 1,ny
          IF (kn2(j,i).le.kmax) THEN
            ...
            C6(j,i) = (thetav(j,i) + dt*rmp*( -C9(j,i)+im*kx(i)*C5(j,i)/pi)) &
    ...
    DO i = ista,iend
      DO j = 1,ny
        IF (kn2(j,i).le.kmax) THEN
          ...
          thetav(j,i) = (thetav(j,i) + dt*rmp*( -C9(j,i)+im*kx(i)*C5(j,i)/pi)) &
          /(1.0d0 + (kappa*(kk2(j,i)+(2.0d0*pi/Wid)**2))*dt*rmp)
        ELSE
          ps(j,i) = 0.0d0
          theta2(j,i) = 0.0d0
  ...
  DEALLOCATE( C1,C3,C2,C4,C5,C6,C7,C8,C9)
  DEALLOCATE( kx,ky )
  DEALLOCATE( kk2 )
