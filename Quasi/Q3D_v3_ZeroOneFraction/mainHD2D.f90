...
PROGRAM HD2D
  ...
  CALL MPI_BCAST(   frac,  1,GC_REAL,      0,MPI_COMM_WORLD,ierr)
  Dkx = 1.0_GP/Lx
  Dky = 1.0_GP/Ly
  ...
  !
    ...
    DO o = ord,2,-1
      ...
      ! have done both of the linear ones now
      ! now reduce the zero one modes in C1
      DO i = ista,iend
        DO j = 1,ny
          IF (i.eq.1.and.j.eq.1) Then
            C7(j,i) = C1(j,i)*frac ! put it into 7 for now as this is the one that is used last
          else
            C7(j,i) = C1(j,i)
          end if
        END DO
